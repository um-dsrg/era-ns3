...
def options(opt):
    ...
    opt.add_option('--enable-gprof',
                   help=('When set build with the required profile flags'),
                   action='store_true', default=False,
                   dest='enable_gprof')
...
def configure(conf):
    ...
    if Options.options.enable_gprof:
        ...
        print("%-41s: %s%s%s" % ("Gprof Enabled (-pg): ", Logs.colors('GREEN'), 'Yes',
                                 Logs.colors('NORMAL')))
    else:
        print("%-41s: %s%s%s" % ("Gprof Enabled (-pg): ", Logs.colors('RED'), 'No',
                                 Logs.colors('NORMAL')))
