...
UnipathReceiver::StartApplication ()
{
  NS_LOG_INFO (Simulator::Now ().GetSeconds ()
               << "s - UnipathReceiver: Flow " << m_id << " started reception");
...
{
  NS_LOG_INFO (Simulator::Now ().GetSeconds ()
               << "s - UnipathReceiver: Flow " << m_id << " stopped reception");
}
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      NS_LOG_INFO (Simulator::Now ().GetSeconds ()
                   << "s - Flow " << m_id << " received " << recvBytes << "bytes of data");
      ...
      NS_LOG_INFO (Simulator::Now ().GetSeconds ()
                   << "s - Flow " << m_id << " extracted " << packetsReceived << " packet(s)");
      ...
      NS_LOG_INFO (Simulator::Now ().GetSeconds ()
                   << "s - Flow " << m_id
                   << " Number of pending bytes remaining: " << pendingBytes);
    }
}
