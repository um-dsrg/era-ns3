...
const std::map<id_t, std::pair<uint64_t, double>>& ApplicationHelper::GetCompressedDelayLog() const {
    return m_flowDelayLog;
}

void ApplicationHelper::CompressDelayLog() {
    ...
    for (auto& receiverApplicationPair : m_receiverApplications) {
        ...
        Ptr<ApplicationBase> receiverApp = DynamicCast<ApplicationBase>(receiverApplicationPair.second);
        NS_ABORT_MSG_IF(receiverApp == nullptr, "The receiver application could not be converted to base");

        Ptr<ApplicationBase> transmitterApp = DynamicCast<ApplicationBase>(m_transmitterApplications.at(flowId));
        NS_ABORT_MSG_IF(transmitterApp == nullptr, "The transmitter application could not be converted to base");
        ...
        std::pair<uint64_t /* number of packets */,
                  double /* total delay */>& flowDelayLog {m_flowDelayLog.at(flowId)};
    ...
    Simulator::Schedule(Time("500ms"), &ApplicationHelper::CompressDelayLog, this);
}
