...
public:
  ...
  struct FlowDetails
  {
    FlowDetails (double requestedGoodput)
        : nBytesReceived (0),
          nPacketsReceived (0),
          requestedGoodput (requestedGoodput),
          goodputAtQuota (0.0)
    {
    }
    ...
    friend std::ostream &
    operator<< (std::ostream &output, FlowDetails &flow)
    {
    ...
    double
        goodputAtQuota; //!< The goodput when the flow has received enough bytes to meet the quota
  ...
  bool
  GetLogGoodputEveryPacket () const
  {
...
private:
  void ReceivePacket (std::string context, ns3::Ptr<const ns3::Packet> packet,
