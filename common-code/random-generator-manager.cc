...
uint32_t RandomGeneratorManager::GetSeed() {
    return m_seed;
}

uint32_t RandomGeneratorManager::GetRun() {
    return m_run;
}

void RandomGeneratorManager::SetSeed (uint32_t seed) {
    m_seed = seed;
}

void RandomGeneratorManager::SetRun (uint32_t run) {
    m_run = run;
}
